 ...
 * Shared by the phone bottom sheet and the desk dialog.
 */
export function DispositionForm({ itemId, onDone }: { itemId: string; onDone: () => void }) {
...
/** Phone bottom sheet around the disposition form. */
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-black/50" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="w-full rounded-t-2xl bg-background p-4 pb-8 text-foreground" onClick={(e) => e.stopPropagation()}>
        ...
        <DispositionForm itemId={itemId} onDone={onDone} />
      </div>
    </div>
  );
}
